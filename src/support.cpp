...
ConVar cv_smoke_grenade_checks_radius ("grenade_checks_radius", "240", "Radius to check for smoke clouds around a detonated grenade.", true, 32.0f, 320.0f);
...
bool BotSupport::isLineBlockedBySmoke (const Vector &from, const Vector &to) {
   ...
   for (auto pent : gameState.getActiveGrenades ()) {
      ...
      const float smokeRadiusSq = cr::sqrf (cv_smoke_grenade_checks_radius.as <float> ());
   ...
   const float maxSmokedLength = 0.7f * cv_smoke_grenade_checks_radius.as <float> ();
